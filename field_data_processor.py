...
class FieldDataProcessor:
    ...
    def apply_corrections(self, column_name='Crop_type', abs_column='Elevation'):
        self.df['Elevation'] = self.df[abs_column].abs()
        self.df['Crop_type'] = self.df[column_name].apply(lambda crop: self.values_to_rename.get(crop, crop))
        self.df['Crop_type'] = self.df[column_name].apply(lambda crop: self.values_to_rename.get(crop, crop))

    def weather_station_mapping(self):
        weather_map_data = read_from_web_CSV(self.weather_map_data)
        self.df = self.df.merge(weather_map_data, on= 'Field_ID', how= 'left' )
